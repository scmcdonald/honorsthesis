...
---
# Set Up
For the data acquisition, four the packages below are needed. Additionally, we load a CSV with the variables needed from the IPEDS tables.
...
The code below acquires the institutional characteristics variables of interest, specifically religious affiliation for 2000-2017. 
```{r, warning = FALSE, message = FALSE}
survey_file <- "ic"
capital_survey_file <- "IC"
extra <- ""
...
We check to ensure that Sector is consistent with CNTLAFFI. For our analysis, we select nonprofit and private only, and fill in CNTLAFFI.
```{r}
...
This is wear we select catholic schools. All schools must have 2017 data.

```{r}

catholic <- data %>% filter(RELAFFIL == "Roman Catholic" & YEAR == 2017) %>% select(UNITID)
catholic.ids <- unique(catholic$UNITID)

catholic <- data[data$UNITID %in% catholic.ids, ]
...
```{r, warning=FALSE}
#write.csv(data, "C:/Users/Sarah McDonald/Documents/honorsthesis/manipulated_data/cath_ipeds_dioceses.csv", row.names = FALSE)
data <- read.csv("C:/Users/Sarah McDonald/Documents/honorsthesis/manipulated_data/cath_ipeds_dioceses.csv")
...
#write.csv(list, "C:/Users/Sarah McDonald/Documents/honorsthesis/raw_data/catholic_hierarchy.csv")
...
```{r, warning=FALSE}
data <- subset(data, select = -c(GeoName))

#write.csv(data, "C:/Users/Sarah McDonald/Documents/honorsthesis/manipulated_data/data_socio.csv", row.names = FALSE)
data <- read.csv("C:/Users/Sarah McDonald/Documents/honorsthesis/manipulated_data/data_socio.csv")
data$county <- ifelse(nchar(data$county) == 4,  paste0("0", data$county), data$county)
```
...
Lexis Nexis
```{r}
arch <- read.csv("C:/Users/Sarah McDonald/Documents/honorsthesis/raw_data/archdioceses-nexis.csv", header = FALSE)
dioceses<- read.csv("C:/Users/Sarah McDonald/Documents/honorsthesis/raw_data/dioceses-nexis.csv", header = FALSE)


arch <- arch %>% select(V2, V3)

colnames(arch)[colnames(arch)== "V2" ] <- "dioceses"
colnames(arch)[colnames(arch)== "V3" ] <- "nexis"

dioceses <- dioceses %>% select(V1, V2)

colnames(dioceses)[colnames(dioceses)== "V1" ] <- "dioceses"
colnames(dioceses)[colnames(dioceses)== "V2" ] <- "nexis"


nexis <- rbind(arch, dioceses)

nexis$dioceses <- as.character(nexis$dioceses)


nexis <- nexis %>% separate(dioceses,c("dioceses", "year"),  sep = "_")



data$Dioceses <- as.character(data$Dioceses)

data$Dioceses[data$Dioceses == "St. Paul &amp; Minneapolis"] <- "St. Paul and Minneapolis"


missing <- read.csv("C:/Users/Sarah McDonald/Documents/honorsthesis/raw_data/missing-nexis.csv")

nexis <- rbind(nexis, missing)

#write.csv(nexis, "C:/Users/Sarah McDonald/Documents/honorsthesis/manipulated_data/nexis.csv")
nexis <- read.csv("C:/Users/Sarah McDonald/Documents/honorsthesis/manipulated_data/nexis.csv")
#check that dioceses match
not_in_cath <- data %>% select(Dioceses) %>% filter(!(Dioceses %in% nexis$dioceses) == TRUE)
not_in_cath <- unique(not_in_cath)


data <- merge(data, nexis, by.x = c("YEAR", "Dioceses"), by.y = c("year", "dioceses"), all.x = TRUE)
...
for(k in 2:length(unique(data$YEAR))){ 
  
  k <- unique(data$YEAR)[k]
    
    for (j in 1:length(unique(data$LINE))){

      j <- unique(data$LINE)[j]

      for (i in 1:length(unique(data$UNITID))){
      
        i <- unique(data$UNITID)[i]
   try(
  data[data$UNITID == i & data$LINE == j & data$YEAR == k, "EFTOTLT_1"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == k-1, "EFTOTLT"])
  try(
  data[data$UNITID == i & data$LINE == j & data$YEAR == k, "EFTOTLM_1"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == k-1, "EFTOTLM"])
 try(
  data[data$UNITID == i & data$LINE == j & data$YEAR == k, "EFHISPT_1"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == k-1, "EFHISPT"])
 try( 
  data[data$UNITID == i & data$LINE == j & data$YEAR == k, "bishop_pub_1"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == k-1, "bishop_pub"]  )

      }
    }
} 

for(l in 3:length(unique(data$YEAR))){ 
  
  l <- unique(data$YEAR)[l]
...
try(
  data[data$UNITID == i & data$LINE == j & data$YEAR == l, "EFTOTLT_2"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == l-2, "EFTOTLT"]
  )
try(
  data[data$UNITID == i & data$LINE == j & data$YEAR == l, "EFTOTLM_2"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == l-2, "EFTOTLM"])
 try(
  data[data$UNITID == i & data$LINE == j & data$YEAR == l, "EFHISPT_2"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == l-2, "EFHISPT"])
try(
 
   data[data$UNITID == i & data$LINE == j & data$YEAR == l, "bishop_pub_2"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == l-2, "bishop_pub"])
...
for(m in 4:length(unique(data$YEAR))){ 
  
  m <- unique(data$YEAR)[m]
    ...
    for (j in 1:length(unique(data$LINE))){
      ...
      for (i in 1:length(unique(data$UNITID))){
        ...
        i <- unique(data$UNITID)[i]
try(
  data[data$UNITID == i & data$LINE == j & data$YEAR == m, "EFTOTLT_3"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == m-3, "EFTOTLT"]
  )
try(
  data[data$UNITID == i & data$LINE == j & data$YEAR == m, "EFTOTLM_3"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == m-3, "EFTOTLM"])
 try(
  data[data$UNITID == i & data$LINE == j & data$YEAR == m, "EFHISPT_3"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == m-3, "EFHISPT"])
 try(
   data[data$UNITID == i & data$LINE == j & data$YEAR == m, "bishop_pub_3"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == m-3, "bishop_pub"]
 )
...
data[data$YEAR == 1990, "EFTOTLT_1"] <- NA
data[data$YEAR == 1990, "EFTOTLM_1"] <- NA
data[data$YEAR == 1990, "EFHISPT_1"] <- NA
data[data$YEAR == 1990, "bishop_pub_1"] <- NA
data[data$YEAR == 1990 | data$YEAR == 1991, "EFTOTLT_2"] <- NA
data[data$YEAR == 1990 | data$YEAR == 1991, "EFTOTLM_2"] <- NA
data[data$YEAR == 1990 | data$YEAR == 1991, "EFHISPT_2"] <- NA
data[data$YEAR == 1990 | data$YEAR == 1991, "bishop_pub_2"] <- NA
data[data$YEAR == 1990 | data$YEAR == 1991| data$YEAR == 1992, "EFTOTLT_3"] <- NA
data[data$YEAR == 1990 | data$YEAR == 1991| data$YEAR == 1992, "EFTOTLM_3"] <- NA
data[data$YEAR == 1990 | data$YEAR == 1991| data$YEAR == 1992, "EFHISPT_3"] <- NA
data[data$YEAR == 1990 | data$YEAR == 1991 | data$YEAR == 1992, "bishop_pub_3"] <- NA

data$bishop_pub_sum_3 <- data$bishop_pub_1 + data$bishop_pub_2 + data$bishop_pub_3
  ...
  k <- unique(data$YEAR)[k]
    ...
    for (j in 1:length(unique(data$LINE))){
      ...
      for (i in 1:length(unique(data$UNITID))){
        ...
        i <- unique(data$UNITID)[i]
        try(
  data[data$UNITID == i & data$LINE == j & data$YEAR == k, "nexis_1"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == k-1, "nexis"]  
        )
...
for(l in 3:length(unique(data$YEAR))){ 
 ...
 try(
   data[data$UNITID == i & data$LINE == j & data$YEAR == l, "nexis_2"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == l-2, "nexis"]
 )
...
for(m in 4:length(unique(data$YEAR))){ 
 ...
 try(
   data[data$UNITID == i & data$LINE == j & data$YEAR == m, "nexis_3"] <-
      data[data$UNITID == i & data$LINE == j & data$YEAR == m-3, "nexis"]
 )
...
data[data$YEAR == 1990, "nexis_1"] <- NA
data[data$YEAR == 1990 | data$YEAR == 1991, "nexis_2"] <- NA
data[data$YEAR == 1990 | data$YEAR == 1991 | data$YEAR == 1992, "nexis_3"] <- NA

data$nexis_sum_3 <- data$nexis_1 + data$nexis_2 + data$nexis_3


```

```{r}
# Three Year Percent Change TOTAL Enrollment

data$EFTOTLT_3_change <- 0

for(k in 4:length(unique(data$YEAR))){ 
  ...
  # if 2016 == 0     
if (data[data$UNITID== i & data$LINE == j & data$YEAR == k-3, "EFTOTLT"] == 0){
  
  #and 2017 ==0, then percent change = 0
  if(data[data$UNITID== i & data$LINE == j & data$YEAR == k, "EFTOTLT"] == 0){
    
    data[data$UNITID == i & data$LINE == j & data$YEAR == k, "EFTOTLT_3_change"] <- 0
    
    # and 2017 = n, then percent change = 100 (so that we do not have major outliers)
  } else {
    data[data$UNITID == i & data$LINE == j & data$YEAR == k, "EFTOTLT_3_change"] <- 100
 
  }
  
  #else, use typical percent chaneg function
} else {
  
       m <- ((data[data$UNITID== i & data$LINE == j & data$YEAR == k, "EFTOTLT"] -  data[data$UNITID== i & data$LINE == j & data$YEAR == k-3, "EFTOTLT"])
   /data[data$UNITID== i & data$LINE == j & data$YEAR == k-3, "EFTOTLT"])*100
  
  data[data$UNITID == i & data$LINE == j & data$YEAR == k, "EFTOTLT_3_change"] <- m
...
data[data$YEAR == 1990, "EFTOTLT_3_change"] <- NA
data[data$YEAR == 1991, "EFTOTLT_3_change"] <- NA
data[data$YEAR == 1992, "EFTOTLT_3_change"] <- NA

data <-  subset(data,select=-c(X))

#write.csv(data, "C:/Users/Sarah McDonald/Documents/honorsthesis/manipulated_data/final_dataset.csv", row.names = FALSE)
