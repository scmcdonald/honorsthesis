...
```{r}
arch <- read.csv("C:/Users/Sarah McDonald/Documents/honorsthesis/raw_data/archdioceses-nexus.csv", header = FALSE)
dioceses<- read.csv("C:/Users/Sarah McDonald/Documents/honorsthesis/raw_data/dioceses-nexus.csv", header = FALSE)


arch <- arch %>% select(V2, V3)

colnames(arch)[colnames(arch)== "V2" ] <- "dioceses"
colnames(arch)[colnames(arch)== "V3" ] <- "nexus"

dioceses <- dioceses %>% select(V1, V2)

colnames(dioceses)[colnames(dioceses)== "V1" ] <- "dioceses"
colnames(dioceses)[colnames(dioceses)== "V2" ] <- "nexus"


nexus <- rbind(arch, dioceses)

nexus$dioceses <- as.character(nexus$dioceses)


nexus <- nexus %>% separate(dioceses,c("dioceses", "year"),  sep = "_")
...
cath_inst$Dioceses <- as.character(cath_inst$Dioceses)

cath_inst$Dioceses[cath_inst$Dioceses == "St. Paul &amp; Minneapolis"] <- "St. Paul and Minneapolis"


missing <- read.csv("C:/Users/Sarah McDonald/Documents/honorsthesis/raw_data/missing-nexus.csv")

nexus <- rbind(nexus, missing)

#write.csv(nexus, "C:/Users/Sarah McDonald/Documents/honorsthesis/manipulated_data/nexus.csv")

#check that dioceses match
not_in_cath <- cath_inst %>% select(Dioceses) %>% filter(!(Dioceses %in% nexus$dioceses) == TRUE)
not_in_cath <- unique(not_in_cath)


cath_inst <- merge(cath_inst, nexus, by.x = c("YEAR", "Dioceses"), by.y = c("year", "dioceses"), all.x = TRUE)


names(cath_inst)[names(cath_inst) == "nexus"] <- "nexis"
  ...
  k <- unique(cath_inst$YEAR)[k]
    ...
    for (j in 1:length(unique(cath_inst$LINE))){
      ...
      for (i in 1:length(unique(cath_inst$UNITID))){
        ...
        i <- unique(cath_inst$UNITID)[i]
  cath_inst[cath_inst$UNITID == i & cath_inst$LINE == j & cath_inst$YEAR == k, "nexis_1"] <-
      cath_inst[cath_inst$UNITID == i & cath_inst$LINE == j & cath_inst$YEAR == k-1, "nexis"]  
...
for(l in 3:length(unique(cath_inst$YEAR))){ 
  ...
  l <- unique(cath_inst$YEAR)[l]
   ...
   cath_inst[cath_inst$UNITID == i & cath_inst$LINE == j & cath_inst$YEAR == l, "nexis_2"] <-
      cath_inst[cath_inst$UNITID == i & cath_inst$LINE == j & cath_inst$YEAR == l-2, "nexis"]
...
for(m in 4:length(unique(cath_inst$YEAR))){ 
  ...
  m <- unique(cath_inst$YEAR)[m]
   ...
   cath_inst[cath_inst$UNITID == i & cath_inst$LINE == j & cath_inst$YEAR == m, "nexis_3"] <-
      cath_inst[cath_inst$UNITID == i & cath_inst$LINE == j & cath_inst$YEAR == m-3, "nexis"]
...
cath_inst[cath_inst$YEAR == 1990, "nexis_1"] <- NA
cath_inst[cath_inst$YEAR == 1990 | cath_inst$YEAR == 1991, "nexis_2"] <- NA
cath_inst[cath_inst$YEAR == 1990 | cath_inst$YEAR == 1991 | cath_inst$YEAR == 1992, "nexis_3"] <- NA

cath_inst$nexis_sum_3 <- cath_inst$nexis_1 + cath_inst$nexis_2 + cath_inst$nexis_3
...
cath_inst$EFTOTLT_3_change <- 0
for(k in 4:length(unique(cath_inst$YEAR))){ 
  ...
  # if 2016 == 0     
if (cath_inst[cath_inst$UNITID== i & cath_inst$LINE == j & cath_inst$YEAR == k-3, "EFTOTLT"] == 0){
  
  #and 2017 ==0, then percent change = 0
  if(cath_inst[cath_inst$UNITID== i & cath_inst$LINE == j & cath_inst$YEAR == k, "EFTOTLT"] == 0){
    
    cath_inst[cath_inst$UNITID == i & cath_inst$LINE == j & cath_inst$YEAR == k, "EFTOTLT_3_change"] <- 0
    
    # and 2017 = n, then percent change = 100 (so that we do not have major outliers)
  } else {
    cath_inst[cath_inst$UNITID == i & cath_inst$LINE == j & cath_inst$YEAR == k, "EFTOTLT_3_change"] <- 100
       ...
       m <- ((cath_inst[cath_inst$UNITID== i & cath_inst$LINE == j & cath_inst$YEAR == k, "EFTOTLT"] -  cath_inst[cath_inst$UNITID== i & cath_inst$LINE == j & cath_inst$YEAR == k-3, "EFTOTLT"])
   /cath_inst[cath_inst$UNITID== i & cath_inst$LINE == j & cath_inst$YEAR == k-3, "EFTOTLT"])*100
  
  cath_inst[cath_inst$UNITID == i & cath_inst$LINE == j & cath_inst$YEAR == k, "EFTOTLT_3_change"] <- m
  
      }
    }
  }
} 
cath_inst[cath_inst$YEAR == 1990, "EFTOTLT_3_change"] <- NA
cath_inst[cath_inst$YEAR == 1991, "EFTOTLT_3_change"] <- NA
cath_inst[cath_inst$YEAR == 1992, "EFTOTLT_3_change"] <- NA
